#!/bin/bash
# Clean rerun in two phases (3 concurrent runs each) so q2a's audio encoding
# never overlaps omni decode: wall times stay comparable within each phase.
set -uo pipefail
...
echo "PHASE q2a"
run 0 30103 q2a_baseline  --target-model $Q2A --dataset aishell &
run 1 30104 q2a_eagle3    --target-model $Q2A --dataset aishell \
    --draft-model yuekai/qwen2_audio_aishell_sft_eagle3 &
run 2 30105 q2a_dflash    --target-model $Q2A --dataset aishell \
